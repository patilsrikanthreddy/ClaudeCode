 ...
 */
function isPONumberUnique(poNumber: string, currentRecordId?: number): boolean {
    try {
...
export function beforeSubmit(context: EntryPoints.UserEvent.beforeSubmitContext): void {
    try {
        ...
        const poNumber = newRecord.getValue({ fieldId: 'tranid' }) as string;
